...
if [[ ! "${_base_date}" =~ ^[0-9]{4}[-/][0-9]{2}[-/][0-9]{2}$ ]]; then
    echo "[030101]エラー: パラメータ2は 'YYYY/MM/DD' または 'YYYY-MM-DD' の形式で指定してください。"
    echo "$(date '+%Y/%m/%d %H:%M:%S') [ERROR] [$(basename $0)] [030101]エラー: パラメータ2は 'YYYY/MM/DD' または 'YYYY-MM-DD' の形式で指定してください。" >> ${LOG_DIR}/data_generator.log
    exit 1
fi
...
if date --version >/dev/null 2>&1; then
    ...
    if [[ -n "${_time_part}" ]]; then
        _generated_date=$(date -d "${_base_date} ${_time_part}" +"${_date_format}" 2>/dev/null)
    else
        _generated_date=$(date -d "${_base_date}" +"${_date_format}" 2>/dev/null)
    fi
else
    # BSD/macOS date
    if [[ -n "${_time_part}" ]]; then
        _generated_date=$(date -j -f "%Y/%m/%d %H:%M:%S" "${_base_date} ${_time_part}" +"${_date_format}" 2>/dev/null)
    else
        _generated_date=$(date -j -f "%Y/%m/%d" "${_base_date}" +"${_date_format}" 2>/dev/null)
    fi
fi

# 日付生成の結果を確認
if [[ -z "${_generated_date}" ]]; then
    echo "[030102]エラー: 入力された日付またはフォーマットが無効です。入力された日付が無効であるか、フォーマットが不正です。"
    echo "$(date '+%Y/%m/%d %H:%M:%S') [ERROR] [$(basename $0)] [030102]エラー: 入力された日付またはフォーマットが無効です。入力された日付が無効であるか、フォーマットが不正です。" >> ${LOG_DIR}/data_generator.log
    exit 1
fi

# 生成された日付を出力
echo "$(date '+%Y/%m/%d %H:%M:%S') [DEBUG] [$(basename $0)] [030001]生成された日付: ${_generated_date}" >> ${LOG_DIR}/data_generator.log
echo "$(date '+%Y/%m/%d %H:%M:%S') [INFO]  [$(basename $0)] END" >> ${LOG_DIR}/data_generator.log
echo "${_generated_date}"
exit 0
