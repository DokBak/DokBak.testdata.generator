...
export COLUMN_DATA_TRIM=""
export COLUMN_DATA_SIGNED=""
export SIGN_DIGIT=""
export DATE_SEPARATOR=""
export TIME_SEPARATOR=""
export SEPARATOR_DATE_TIME=""
export REFERENCE_DATE=""
export CASE_MODE=""
export MULTIBYTE_STRING=""
...
while IFS="=" read -r _key _value; do
    case "${_key}" in
        ...
        COLUMN_DATA_LENGTH)
            ...
            ;;
        COLUMN_DATA_TRIM)
            COLUMN_DATA_TRIM="${_value//\"/}"
            ;;
        COLUMN_DATA_SIGNED)
            COLUMN_DATA_SIGNED="${_value//\"/}"
            ;;
        SIGN_DIGIT)
            SIGN_DIGIT="${_value//\"/}"
            ;;
...
echo "$(date '+%Y/%m/%d %H:%M:%S') [DEBUG] [$(basename $0)] COLUMN_DATA_TRIM=${COLUMN_DATA_TRIM}" >> ${LOG_DIR}data_generator.log
echo "$(date '+%Y/%m/%d %H:%M:%S') [DEBUG] [$(basename $0)] COLUMN_DATA_SIGNED=${COLUMN_DATA_SIGNED}" >> ${LOG_DIR}data_generator.log
echo "$(date '+%Y/%m/%d %H:%M:%S') [DEBUG] [$(basename $0)] COLUMN_DATA_TRIM=${COLUMN_DATA_TRIM}" >> ${LOG_DIR}data_generator.log
...
echo "$(date '+%Y/%m/%d %H:%M:%S') [DEBUG] [$(basename $0)] SEPARATOR_DATE_TIME=${SEPARATOR_DATE_TIME}" >> ${LOG_DIR}data_generator.log
...
echo "$(date '+%Y/%m/%d %H:%M:%S') [DEBUG] [$(basename $0)] MULTIBYTE_STRING=${MULTIBYTE_STRING}" >> ${LOG_DIR}data_generator.log
...
echo "$(date '+%Y/%m/%d %H:%M:%S') [DEBUG] [$(basename $0)] DATABASE_TABLERIGHT=${DATABASE_TABLERIGHT}" >> ${LOG_DIR}data_generator.log
