...
#
#  実行方法    　:
#     sh ./script/db_output_compression_format.sh
#
...
case "${FILE_COMPRESSION}" in
    "NONE")  # 圧縮しない
        echo "  出力ファイルの中身確認"
        cat "${FILES_DIR}${FILE_NAME}${FILE_EXTENSION}"
        ...
        echo "$(date '+%Y/%m/%d %H:%M:%S') [DEBUG] [$(basename $0)] " `ls -l "${FILES_DIR}${FILE_NAME}${FILE_EXTENSION}"` >> ${LOG_DIR}data_generator.log
        echo "$(date '+%Y/%m/%d %H:%M:%S') [DEBUG] [$(basename $0)] 出力ファイルの中身確認 " >> ${LOG_DIR}data_generator.log
        echo "$(date '+%Y/%m/%d %H:%M:%S') [DEBUG] [$(basename $0)] " `cat "${FILES_DIR}${FILE_NAME}${FILE_EXTENSION}"` >> ${LOG_DIR}data_generator.log
        ;;
    ".gz")  # gzip形式で圧縮
    ...
    ".tar")  # tar形式で圧縮
        ...
        tar -cvf "${FILES_DIR}${FILE_NAME}${FILE_EXTENSION}${FILE_COMPRESSION}" -C "${FILES_DIR}" "${FILE_NAME}${FILE_EXTENSION}"
